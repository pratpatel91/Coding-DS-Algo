public class DP_Optimized_LongestCommonSubSequence
{
	...
	public static void main(String[] args)
	{
		...
		System.out.println("longest char: " + optimized_LCS(s1, s2));
	}
	...
	// SPACE Optimized
	// SC=O(2*N) current rows data = f(previous rows data is aggregation of all rows
	// above it)
	public static int optimized_LCS(String s1, String s2)
	{
		int[][] dp = new int[2][s1.length() + 1];

		for (int currS1Len = 0; currS1Len <= s1.length(); currS1Len++)
		{
			for (int currS2Len = 0; currS2Len <= s2.length(); currS2Len++)
			{
				// base cases (populating cacheMatrix with Os)
				if (currS1Len == 0 || currS2Len == 0)
				{
					//currS1Len%2 avoids index out of bounds since dp[2][s.length]
					dp[currS1Len%2][currS2Len] = 0;
				}
				else if (s1.charAt(currS1Len - 1) == s2.charAt(currS2Len - 1))
				{
					//currS1Len%2 avoids index out of bounds since dp[2][s.length]
					dp[currS1Len % 2][currS2Len] = dp[(currS1Len + 1) % 2][currS2Len - 1] + 1;
				}
				else
				{
					//currS1Len%2 avoids index out of bounds since dp[2][s.length]
					dp[currS1Len%2][currS2Len] = Math.max(dp[(currS1Len + 1) % 2][currS2Len],
							dp[currS1Len%2][currS2Len - 1]);
				}
			}
		}
		//answer exists at last element of row 1 or last element of row 2 
		return Math.max(dp[0][s1.length()],dp[1][s1.length()]) ; // bottom right cell contains answer
	}
