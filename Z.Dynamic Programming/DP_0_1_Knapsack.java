public class DP_0_1_Knapsack
{
	...
	public static void main(String[] args)
	{
		...
		calls = 0;
		System.out.print("\n" + memoMaxProfixKnapsack(values, weights, values.length, capacity) + "; called "
				+ calls + "x\n");

		System.out.println("\nProfit:" + tabMaxProfixKnapsack(values, weights, values.length, capacity));
	}
	...
	public static int memoMaxProfixKnapsack(int[] values, int[] weights, int index, int capacity)
	{
		if (capacity == 0 || index == 0)
		{
			return 0;
		...
		if (capacity < weights[index - 1])
		{
			calls++;
			// excluding (changing index only)
			int profit = memoMaxProfixKnapsack(values, weights, index - 1, capacity);
		...
		else
		{
			...
			int profit = Math.max(memoMaxProfixKnapsack(values, weights, index - 1, capacity), values[index - 1]
					+ memoMaxProfixKnapsack(values, weights, index - 1, capacity - weights[index - 1]));
			cacheMatrix[index - 1][capacity] = profit;
			return profit;
		}
	}
	...
	// TC=O(N^2)
	// SC=O(N^2) due to cache matrix
	public static int tabMaxProfixKnapsack(int[] values, int[] weights, int index, int capacity)
	{
		if (capacity == 0 || index == 0)
		{
			return 0;
